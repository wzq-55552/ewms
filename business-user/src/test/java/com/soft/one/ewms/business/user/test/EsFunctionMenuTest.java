...
@SpringBootTest
@Log4j2
public class EsFunctionMenuTest {
    ...
    public void testSaveEs(){
        ...
        functionMenus.forEach(functionMenu -> {
            // 字段都在某一表，那数据库类也行
            Index.Builder builder = new Index.Builder(functionMenu);
            builder.refresh(true);
            // 添加到INDEX_OPERATION索引
            Index index = builder.index(EsConstant.INDEX_FUNCTION_MENU).type(EsConstant.DEFAULT_DOC).build();
            try {
